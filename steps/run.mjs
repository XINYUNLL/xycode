...
// Run TypeScript via its JS entry, not the .bin/tsc shim: that shim is a POSIX
// shell script, and Node on Windows (>=20, CVE-2024-27980) refuses to spawn it
// directly — spawnSync then fails with ENOENT, leaving status null and
// stdout/stderr undefined, which the error path concatenates into "NaN".
const TSC = join(REPO, "node_modules", "typescript", "lib", "tsc.js");
const runTsc = (args, opts) => spawnSync(process.execPath, [TSC, ...args], opts);
...
if (flag("--live")) {
  ...
  const b = runTsc(["--module", "nodenext", "--moduleResolution", "nodenext", "--target", "es2022", "--skipLibCheck", "--outDir", tsDir, join(tsDir, "cli.ts")], { stdio: "inherit", env });
...
const b = runTsc(["--module", "nodenext", "--moduleResolution", "nodenext", "--target", "es2022", "--skipLibCheck", "--outDir", tsDir, join(tsDir, entry)], { encoding: "utf-8" });
...
await mock.close();
verify();
// Windows: let the event loop drain so undici's streaming sockets close cleanly
// (a hard process.exit(0) races them and libuv asserts UV_HANDLE_CLOSING). Keep
// a short force-exit safety net for steps that leave a child process (e.g. MCP)
// whose stdio would otherwise keep the loop alive. The unref() means the timer
// alone never holds the process open — a clean step still exits immediately.
process.exitCode = 0;

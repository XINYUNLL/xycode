...
function parseArgs(): ParsedArgs {
  ...
  let thinking = false;
  let model = process.env.XYCODE_MODEL || "claude-opus-4-6";
...
Options:
  ...
  --model, -m         Model to use (default: claude-opus-4-6, or XYCODE_MODEL env)
...
Examples:
  xycode "fix the bug in src/app.ts"
  xycode --yolo "run all tests and fix failures"
  xycode --plan "how would you refactor this?"
  xycode --accept-edits "add error handling to api.ts"
  xycode --max-cost 0.50 --max-turns 20 "implement feature X"
  OPENAI_API_KEY=sk-xxx xycode --api-base https://aihubmix.com/v1 --model gpt-4o "hello"
  xycode --resume
  xycode  # starts interactive REPL
`);
      process.exit(0);
    } else {
...
// Load .env into process.env. Search order: the current directory (project-
// specific) then the agent's own directory (repo root, i.e. dist/..). A key that
// is already set — by the environment or by an earlier location — is never
// overwritten, so cwd beats repo root, and a real env var beats both. Hand-rolled
// to avoid a dotenv dependency; previously the built CLI never read .env (only
// steps/run.mjs --live did), so the repo's .env.example was misleading. Supports
// "#" comments and quoted values.
function loadEnvFile(): void {
  const here = dirname(fileURLToPath(import.meta.url));
  const repoRoot = dirname(here);
  const candidates = [join(process.cwd(), ".env"), join(repoRoot, ".env")];
  for (const envPath of candidates) {
    ...
    for (const rawLine of readFileSync(envPath, "utf-8").split("\n")) {
      ...
      // Standard dotenv semantics: an already-set env var wins over the file.

...
export class Agent {
  ...
  constructor(options: AgentOptions = {}) {
    ...
    // XYCODE_SDK_MAX_RETRIES=0 to isolate our withRetry() in tests (or to
    // opt out of double-retrying in production).
    const sdkRetries =
      process.env.XYCODE_SDK_MAX_RETRIES != null && process.env.XYCODE_SDK_MAX_RETRIES !== "" &&
      !Number.isNaN(Number(process.env.XYCODE_SDK_MAX_RETRIES))
        ? { maxRetries: Number(process.env.XYCODE_SDK_MAX_RETRIES) }
        : {};
    if (this.useOpenAI) {
  ...
  private persistLargeResult(toolName: string, result: string): string {
    ...
    const dir = join(homedir(), ".xycode", "tool-results");
